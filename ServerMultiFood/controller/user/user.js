...
exports.users = async (req, res, next) => {
    ...
    if (req.method == "POST") {
        let newUser = new users.userModel();
    ...
    const totalItemsCount = await users.userModel.countDocuments();
    let listUsers = await users.userModel.find().skip(offset).limit(perPage);
    ...
    if (req.query.hasOwnProperty("_sort")) {
        listUsers = await users.userModel.find().limit(perPage).skip(offset).sort(
            {
    ...
    const formatUser = listUsers.map((item) => ({
        ...item.toObject(),
...
exports.deleteUser = async (req, res, next) => {
    try {
        await users.userModel.deleteOne({ _id: req.params.id });
